...
export default function DevPage() {
  ...
  const [memberTabKey] = useState<string>("members");
    ...
    <div className="min-h-screen p-8" style={{ backgroundColor: "var(--color-bg)" }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 border-b pb-2" style={{ borderColor: "var(--color-border)" }}>
          ...
          <button
            onClick={() => setActiveTab(memberTabKey as TabType)}
            className="px-4 py-2 font-medium transition-colors rounded-t"
            style={{
              color: activeTab === memberTabKey ? "var(--color-primary)" : "var(--color-muted)",
              backgroundColor: activeTab === memberTabKey ? "rgba(128, 128, 128, 0.1)" : "transparent",
            }}
          >
            Members
          </button>
        ...
        <div>
          ...
          {activeTab === memberTabKey && <MembersDevTab />}
        </div>
      </div>
    </div>
  );
// MembersDevTab: Editable table of all member database info
function MembersDevTab() {
  ...
  async function loadMembers() {
    ...
    try {
      ...
      const sorted = (data.members || []).sort((a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
      setMembers(sorted);
    } finally {
      setLoading(false);
    }
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b" style={{ borderColor: "var(--color-border)", backgroundColor: "rgba(128, 128, 128, 0.1)" }}>
                <th className="text-left px-4 py-3 text-sm font-medium" style={{ color: "var(--color-foreground)" }}>Name</th>
                <th className="text-left px-4 py-3 text-sm font-medium" style={{ color: "var(--color-foreground)" }}>Role</th>
                <th className="text-left px-4 py-3 text-sm font-medium" style={{ color: "var(--color-foreground)" }}>Kicked</th>
                <th className="text-left px-4 py-3 text-sm font-medium" style={{ color: "var(--color-foreground)" }}>Kick Date</th>
                ...
                <tr key={member.id} className="border-b" style={{ borderColor: "var(--color-border)", backgroundColor: idx % 2 === 0 ? "transparent" : "rgba(128, 128, 128, 0.03)" }}>
                  ...
                  <td className="px-4 py-3 text-sm" style={{ color: "var(--color-foreground)" }}>
                    {editingId === member.id ? (
                      <input
                        type="datetime-local"
                        value={editFields.created_at ? new Date(editFields.created_at).toISOString().slice(0,16) : ""}
                        onChange={e => setEditFields({ ...editFields, created_at: new Date(e.target.value).toISOString() })}
